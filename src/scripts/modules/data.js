...
function getCategory(category) {
  ...
  xmlhttp.onreadystatechange = () => {
    if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
      ...
      let elements = ""
      for (let i = 0; i < response.categories.length && i < array.length; i++) {
        elements +=
          `<div class="diagram-double-bars" data-label="` + array[i] + `" data-total="` + response.categories[i].total + `">` +
          `<p class="diagram-label">` + categoryNames[response.categories[i].id] + `</p>` +
          `<p class="diagram-total">` + response.categories[i].total + `</p>` +
